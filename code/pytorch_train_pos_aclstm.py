...
def train(dances, frame_rate, batch, seq_len, read_weight_path, write_weight_folder,
          write_bvh_motion_folder, in_frame, out_frame, hidden_size=1024, 
          num_iterations_for_session=100000, resume_from_iteration_num=0):
    ...
    for i in range(num_iterations_for_session):   
        iteration = resume_from_iteration_num + i
...
def main():
    ...
    parser.add_argument('--read_weight_path', type=str, default="", help='Checkpoint model path to resume training from')
    ...
    parser.add_argument('--total_iterations', type=int, default=100000, help='Number of iterations to run in this training session')

    args = parser.parse_args()

    resume_from_iteration_val = 0
    if args.read_weight_path:
        try:
            checkpoint_filename = os.path.basename(args.read_weight_path)
            # Expects format like "0029000.weight"
            iteration_str = checkpoint_filename.split('.')[0]
            if iteration_str.isdigit():
                # The loaded checkpoint was saved after this iteration number was completed.
                # So, the next iteration to run is int(iteration_str) + 1.
                resume_from_iteration_val = int(iteration_str) + 1
                print(f"Resuming training from iteration {resume_from_iteration_val} (loaded checkpoint: {checkpoint_filename})")
            else:
                print(f"Warning: Could not parse iteration number from checkpoint filename '{checkpoint_filename}'. Starting iterations from 0.")
        except Exception as e:
            print(f"Warning: Error parsing iteration number from '{args.read_weight_path}': {e}. Starting iterations from 0.")
    ...
    train(dances, args.dance_frame_rate, args.batch_size, args.seq_len, args.read_weight_path, args.write_weight_folder,
          args.write_bvh_motion_folder, args.in_frame, args.out_frame, args.hidden_size, 
          num_iterations_for_session=args.total_iterations, resume_from_iteration_num=resume_from_iteration_val)
